...
signature.width = CANVAS_SIZE;
signature.height = CANVAS_SIZE;
...
function handleMirror() {
    ...
    const reflectObj = document.querySelector('.reflect-img');

    // 패턴 만들기
    imageObj.onload = function() {
        // ctx.clearRect(0,0,700,700);
        ctx.scale(-1, 1);        
        ctx.drawImage(imageObj, 0, 0, -700, 700);
        reflectObj.src = canvas.toDataURL();
    }

    reflectObj.onload = function() {
        console.log('test');
        ctx2.translate(350, -145);
        ctx2.rotate((Math.PI / 180) * 45);
        ctx2.drawImage(reflectObj, 0, 0, 700, 700);
        ctx2.translate(350, -145);
        ctx2.rotate((Math.PI / 180) * 45);
        ctx2.drawImage(reflectObj, 0, 0, 700, 700);
        ctx2.translate(350, -145);
